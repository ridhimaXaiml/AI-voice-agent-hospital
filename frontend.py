...
if st.button("Cancel appointments"):
    ...
    try:
        ...
        st.success(f"Cancled:{data.get('canceled_count',0)}")
        st.rerun()
...
if st.button("Check appointments"):
    try:
        ...
        resp = requests.post(f"{base_url}/list_appointment/", params=params, timeout=10)
        resp.raise_for_status()
        st.dataframe(
            resp.json(),
            width="stretch",
        ...
        st.warning(f"Could not load appointmnets: {exc}")
